...
def plot_nine(input_coords, target_phase, output_phi_d, test_loss, train_loss, LS_test_loss, Phi_LS, CLAMP_EPSILON):
    ...
    for k in range(m):
        for h in range(n):
            ...
            print('\nLeast Squares output phase: '+str(phi_d_LS))
            ...
            print('Total phase loss between nn and truth: '+str(phase_loss_nn))
            print('Total phase loss between LS and truth: '+str(phase_loss_LS)+'\n')
            ...
            for i in range(600): 
                ...
                X_ls[i] = c_x*cos(Phi_c[i] + phi_d_LS) + b_x
                Y_ls[i] = c_y*cos(Phi_c[i] - phi_d_LS) + b_y
    ...
    plot_title = 'NN(r) vs. LS(y) vs. Truth(b). Test Loss: '+test_loss_str+'\nLS Test Loss: '+LS_test_loss_str
