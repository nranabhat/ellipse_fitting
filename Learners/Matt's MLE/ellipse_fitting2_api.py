...
def cost(phi_contrast_guesses, points, num_atoms):
    """Cost function - returns the log likelihood of the probability distribution
    produced by a given phi and contrast
    """
    phi = phi_contrast_guesses[0]
    contrast = phi_contrast_guesses[1]
    amp = contrast / 2
...
def main(points, num_atoms):
    ...
    -------
    float
        Maximum likelihood estimator for phi, contrast
    """
    ...
    # Use the data range to give us a good contrast guess
    max_coord = np.max(points)
    contrast_guess = (max_coord - 0.5) * 2

    res = minimize(
        cost,
        np.array([corr_phi, contrast_guess]),
        args=(points, num_atoms),
        bounds=((0, np.pi / 2), (0,1)),
    )
    opti_phi = res.x[0]
    opti_contrast = res.x[1]
